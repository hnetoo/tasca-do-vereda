...
const tabs = [
  { name: 'GERAL', path: '/sistema/geral' },
  { name: 'FISCAL', path: '/sistema/fiscal' },
  { name: 'NUVEM', path: '/sistema/nuvem' },
  { name: 'UTILIZADORES', path: '/sistema/utilizadores' },
  { name: 'SAÚDE', path: '/sistema/sistema-health' },
];
...
export function SystemTabs() {
  ...
  return (
    <div className="flex items-center gap-1 border-b border-white/10 mb-8 overflow-x-auto pb-1">
      {tabs.map((tab) => {
        const isActive = pathname === tab.path || (tab.path === '/sistema/geral' && pathname === '/sistema');
