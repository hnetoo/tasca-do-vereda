 ...
 */
export const resolveCategoryId = (dish: Dish | Product, menu_categories: MenuCategory[]): string | null => {
  const catId = (dish as any).category_id || (dish as any).categoryId;
  
  // 1. Direct ID match
  const byId = menu_categories.find(c => norm(catId) === norm(c.id));
  if (byId) return byId.id;

  // 2. By Category Name in Dish
  const byNameFromDish = menu_categories.find(c => norm((dish as any).categoryName) === norm(c.name));
  if (byNameFromDish) return byNameFromDish.id;

  // 3. By ID acting as Name
  const byNameFromId = menu_categories.find(c => norm(c.name) === norm(catId));
  if (byNameFromId) return byNameFromId.id;

  // 4. By Slug
  const bySlug = menu_categories.find(c => slug(c.name) === norm(catId));
...
export const validateDishCategory = (
  ...
  const resolved = resolveCategoryId(dish, menu_categories);
...
export const analyzeAndFixMenu = (
  ...
  const report: MenuConsistencyReport = {
    ...
    fixedProducts: 0,
    orphanedProducts: [],
    missingmenu_categories: []
  };

  const validCategoryIds = new Set(menu_categories.map(c => c.id));
  const fixedMenu = menu.map(product => {
    ...
    const resolvedId = resolveCategoryId(product, menu_categories);
    ...
    if (currentCatId) report.missingmenu_categories.push(currentCatId);
