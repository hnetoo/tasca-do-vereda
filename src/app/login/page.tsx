...
const LoginForm = () => {
  ...
  useEffect(() => {
    const initSecurity = async () => {
        ...
        setLastStoredUserId(lastSelectedUserId);

        if (lastSelectedUserId) {
            const user = users.find((u: UserType) => u.id === lastSelectedUserId);
            if (user) {
                setSelectedUser(user);
            } else {
                localStorage.removeItem('last_selected_user_id'); // Clear if user not found
                setLastStoredUserId(null);
            }
        }
    }
    initSecurity();
              ...
              {loginMode === 'password' ? (
                <form onSubmit={handlePasswordSubmit} className="animate-in fade-in slide-in-from-right-4 duration-500">
                 ...
                 <div className="animate-in fade-in slide-in-from-left-4 duration-500">
                   {lastStoredUserId && users.find((u: UserType) => u.id === lastStoredUserId) && (
                     <div className="mb-6 p-4 rounded-xl bg-primary/10 border border-primary/20 animate-in fade-in slide-in-from-top-2">
                       ...
                       <button
                         onClick={() => handleUserSelect(lastStoredUserId!)}
                         disabled={isProcessing}
                       ...
                       >
                         Continuar como {users.find((u: UserType) => u.id === lastStoredUserId)?.name}
