...
const POS = () => {
  ...
  useEffect(() => {
    if (activeTableId && activeOrders && activeOrders.length > 0) {
      const tableOrder = activeOrders.find((o: Order) => o.tableId === activeTableId && o.status === 'ABERTO');
      if (tableOrder && tableOrder.id !== activeOrderId) {
        console.log('Found order for table:', tableOrder.id);
      } else if (!tableOrder) {
        // setActiveOrder(null);
      }
    }
  ...
  const normalizeClosedOrdersShift = useCallback(async () => {
    try {
      ...
      const toFix = (state.activeOrders || []).filter((o: Order) => {
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-3">
                {cartItems && cartItems.length > 0 ? (
                  cartItems.map((item: OrderItem, idx: number) => { 
                    const dish = menu.find((p: Product) => p.id === item.dishId); 
                    if (!dish) return null; // Silently skip if dish not found
                    ...
                    {(['NUMERARIO', 'TPA', 'TRANSFERENCIA', 'QR_CODE', 'SPLIT'] as PaymentMethod[]).map(method => {
                      ...
                      return (
                        <button 
                          ...
                          disabled={isDisabled}
                          className={`w-full p-4 rounded-xl border-2 text-center font-bold uppercase tracking-wider transition-all duration-200
                            ${isDisabled 
