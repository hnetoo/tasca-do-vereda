...
-- 1. Criar tabela menu_items que está a faltar
...
-- 2. Verificar e corrigir tabela daily_analytics
DO $$
BEGIN
    -- Verificar se a tabela daily_analytics existe
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'daily_analytics' AND table_schema = 'public') THEN
        -- Verificar se a coluna updated_at existe
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'daily_analytics' AND column_name = 'updated_at' AND table_schema = 'public') THEN
            -- Adicionar coluna updated_at se não existir
            ALTER TABLE daily_analytics ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
        END IF;
    END IF;
END $$;

-- 3. Inserir menu_items básicos
...
-- 4. Criar despesas (agora deve funcionar)
