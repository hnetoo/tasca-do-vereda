// TESTE FINAL - Fino + Order com JSONB correto
// Execute no console do browser

import { directSupabaseService } from './src/services/directSupabaseService.js';

console.log('🚀 TESTE FINAL - Fino + Order com JSONB correto');
...
// =====================================================
// 📋 TESTE 2: FINALIZAR PEDIDO COM JSONB CORRETO
// =====================================================
async function testFinalizeOrder() {
  ...
  try {
    ...
    // 2. Criar array de itens (em memória)
    const itemsArray = [
      {
        id: crypto.randomUUID(),
        name: 'Fino',
    ...
    // 3. Criar pedido com items como JSONB string
    const newOrder = {
      id: crypto.randomUUID(),
      ...
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString(),
      items: JSON.stringify(itemsArray) // 🔥 JSONB STRING CORRETO
    };
    
    console.log('🔧 DADOS DO PEDIDO:');
    console.log('🔧 COLUNAS ENVIADAS:', Object.keys(newOrder));
    console.log('🔧 items ARRAY (memória):', itemsArray);
    console.log('🔧 items JSONB (banco):', newOrder.items);
    console.log('🔧 items tipo:', typeof newOrder.items);
    
    // 4. Inserir pedido no Supabase
    const { data, error } = await supabase
      .from('orders')
      .insert(newOrder)
      .select()
    ...
    console.log('📋 items salvos:', data.items);
    console.log('📋 items tipo no banco:', typeof data.items);
...
async function runFinalTest() {
  ...
  if (allSuccess) {
    ...
    console.log('✅ Order finalizada com closedAt');
    console.log('✅ items como JSONB string correto');
