-- Fix cash_shifts schema - Add missing 'notes' column - VERSION 2
...
-- Step 5: Test insert to verify schema works (safe test)
DO $$
BEGIN
    -- Generate a valid UUID for testing
    DECLARE test_uuid UUID := gen_random_uuid();
    DECLARE test_exists BOOLEAN;
    
    -- Check if test record exists first (using valid UUID)
    SELECT EXISTS(SELECT 1 FROM cash_shifts WHERE id = test_uuid::text) INTO test_exists;
    ...
    IF NOT test_exists THEN
        ...
        DELETE FROM cash_shifts WHERE id = test_uuid::text;
        
        RAISE NOTICE 'Test record cleaned up successfully';
    ELSE
        RAISE NOTICE 'Test record already exists, skipping insert test';
    END IF;
END $$;

-- Step 6: Final verification
SELECT 
    'cash_shifts schema fix v2 completed successfully' as status,
